...
class AllUsersView(object):
    ...
    @serialize_exceptions
    def post(self ,department,firstname=None,lastname=None,college_name=None,email=None,role=None,username=None,password=None,about_me=None,phone=None,skill=[] ):

        #skillm =  ast.literal_eval(skill)
       
        user = self.create_new_user_interactor.set_params(firstname=firstname,lastname=lastname,college_name=college_name,email=email,role=role,username=username,password=password,about_me=about_me,phone=phone,skill=skill,department=department).execute()
        body = UserSerializer.serialize(user)
        status = 201
