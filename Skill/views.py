...
from useraccount.serializers import SkillSerializer,MultipleSkillSerializer

# Create your views here.

class AllSkillView(object):
    def __init__(self, get_all_skill_interactor = None, create_new_skill_interactor = None):
        self.get_all_skill_interactor = get_all_skill_interactor
    ...
    @serialize_exceptions
    def post(self, name =None , description=None, technology=None,ormuser=None ):
        # if audience :
        #     audience_list =  ast.literal_eval(audience)
        # else:
        #     audience_list = []
       
        skill = self.create_new_skill_interactor.set_params( name =name , description=description, technology=technology,ormuser=ormuser ).execute()
...
class AllSkillByUserView(object):
    ...
    @serialize_exceptions
    def get(self, ormuser):
        skill = self.get_all_skill_by_user_interactor.set_params(ormuser = ormuser).execute()
